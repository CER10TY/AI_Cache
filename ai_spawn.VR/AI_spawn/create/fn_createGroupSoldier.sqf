/*
	Author: tryteyker
	Description: This function creates a group of soldiers based on params passed on. By default uses macros defined in \ai_spawn\macros.hpp.

	PARAMETERS:
		0: NUMBER - Actual group size (eg: 6 would create 1 leader and 5 group members)
...
if (_unitTypes isEqualTo []) then {
	_unitTypes = ([_side] call SCW_S_fnc_grabUnitPool) select 0;
};
...
for "_i" from 1 to _groupSize do {
	_group createUnit [_unitType, _groupPos, [], 0, "FORM"]; // Creates unit at _groupPos, assigns it to _group.
};
